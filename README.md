...
task genZip(type: Zip) {
    ...
    from('.'){
        ...
        exclude 'pipeline-diagrams'
        exclude '.gitignore'
        exclude '.gitpod.yml'
        exclude 'JenkinsfileParallel'
        exclude 'LICENSE.md'
        exclude 'odsoft_2022-2023_project_pa2_self-assessment_v1.0.pdf'
        exclude 'package.json'
        exclude 'package-lock.json'
        exclude 'parallel_pipeline.png'
        exclude 'sequential_pipeline.png'
        exclude 'settings.gradle'
        exclude 'tsconfig.json'
        exclude 'types.d.ts'
        exclude 'vite.config.ts'
        exclude 'vite.generated.ts'
        exclude 'README.md'
        exclude 'bin'
    }
}
...
We should start by adding the cucumber plugins and configurations with the following lines of code.
...
public  void  i_submit_the_form_with_the_name_and_the_adress(String  name, String  address) {
	...
	new  WebDriverWait(driver, ofSeconds(30), ofSeconds(10))
			.until(ExpectedConditions.elementToBeClickable(customerAddressField));
...
Apache JMeter is an open-source load testing tool that is used to test the performance of web applications and APIs. It can simulate a large number of users concurrently accessing a website or application, which can help identify performance issues such as bottlenecks, errors, and slow response times.

JMeter is a Java-based tool, so it can be run on any system that has a Java Runtime Environment (JRE) installed. It has a graphical user interface (GUI) that allows users to create and configure test plans, define test scenarios, and analyze the results.
JMeter can be used to test the performance of various types of applications, including web applications, databases, FTP servers, and more. It supports a wide range of protocols, including HTTP, HTTPS, JDBC, LDAP, and more.
